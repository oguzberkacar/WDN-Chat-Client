...
//actions
import { setconversationNameInOpenChat, setActiveChatId } from '../../../redux/actions';
				...
				<div>
					...
					<SimpleBar style={{ maxHeight: '100%' }} className="p-4 chat-message-list chat-group-list">
						<ul className="list-unstyled chat-list">
							{state.map((group, key) => (
								<li key={key} className={group.chat_id === props.activeChatId ? 'active' : ''}>
									<Link to="#" onClick={(e) => openUserChat(e, group)}>
										<div className="d-flex align-items-center">
											<div className="chat-user-img me-3 ms-0">
												<div className="avatar-xs">
													<span className="avatar-title rounded-circle wdncolor text-white">{group && group.showname.charAt(0).toUpperCase()}</span>
												</div>
											</div>
											<div className="flex-grow-1 overflow-hidden">
												<h5 className="text-truncate font-size-14 mb-0">{group.showname}</h5>
											</div>
										</div>
									</Link>
								</li>
