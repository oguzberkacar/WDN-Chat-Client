...
import { setOnlineUsers, addLoggedinUser, activeUser, setActiveChatId, addNewChat } from '../../../redux/actions';
...
const OnlineUsers = (props) => {
	...
	return (
		<React.Fragment>
			...
			<div>
				<ScrollContainer id="container" className="px-4 pb-4 dot_remove d-flex gap-2 scrollflow w-100" dir="ltr">
					{onlineUserList !== null &&
						onlineUserList !== undefined &&
						onlineUserList.map((user, index) => {
							return (
								<div className="item  position-relative" key={index}>
									<Link onClick={() => openUserChat(user)} to="#" className="user-status-box">
										{user.avatar !== null && user.avat !== undefined ? (
											<div className="avatar-xs mx-auto chat-user-img online">
												<img
													src={user.avatar}
													alt="user-img"
													className={`onlineuseravatar ${props.mode == 'dark' ? 'avatarBW' : 'avatarBB'}`}
													onError={() => {
											...
											<div className="avatar-xs mx-auto chat-user-img online">
												<span className={`avatar-title rounded-circle wdncolor text-white ${props.mode == 'dark' ? 'avatarBW' : 'avatarBB'}`}>{user.showname ? user.showname.charAt(0).toUpperCase() : 'X'}</span>
											</div>
										)}

										<div className="onlinew">
											<h5 className="font-size-13 text-truncate mt-3 mb-1">{user.showname ? user.showname : 'No Name'}</h5>
											{user.user_id}
										</div>
									</Link>
								</div>
