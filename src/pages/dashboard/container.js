...
import { getUser, isLogining, setchatList, updateChatListMessage, setActiveChatId, stopLoading } from '../../redux/actions';
...
function DashboardContainer(props) {
			...
			.then((result) => {
				...
				if (obj.status === 200 && obj.success == true) {
					...
					chatrooms.map((i, index) => {
						getChatroomMessages(i.chat_id, i.name, index, i.chat_room_match.chat_room_id);
					});
				}
			})
	...
	async function getChatroomMessages(chatid, name, index, chat_room_id) {
		...
		await fetch(`https://chat.spapp.click/api/chatRoomMessages?chat_room_chat_id=${chatid}`, requestOptions)
			.then((response) => response.text())
			.then((result) => {
				let obj = JSON.parse(result);
		...
		await fetch(`https://api.devapp.one/ads/detail/${id}`, requestOptions)
			...
			.then((result) => {
				...
				if (adsDetail.status === 200 && adsDetail.success === true) {
					// check gender_id
					let Gender = adsDetail.result.gender_id !== undefined ? adsDetail.result.gender_id : 'No Gender Info';

					let Showname = adsDetail.result.showname !== undefined ? adsDetail.result.showname : adsDetail.result.username;

					let Country = adsDetail.result.get_locations !== undefined ? adsDetail.result.get_locations[0].location_details.code : 'No Country';

					// checking user city
					let City = adsDetail.result.get_locations !== undefined ? adsDetail.result.get_locations[1].location_details.location_contents[0].name : 'No City';
					// checking user age
					let Age = adsDetail.result.ads_meta[0] ? adsDetail.result.ads_meta[0].ads_options[0].title : 'No Age';
					// checking user avatar
					let Imageurl = adsDetail.result.ads_photo_profile == null ? 'placeholder' : `${url}/${adsDetail.result.user.id}/400/600/jpeg/${adsDetail.result.ads_photo_profile.profile_photo.file_name}`;
				...
				} else if (adsDetail.status === 200 && adsDetail.success === false && adsDetail.message === 'ads_not_found') {
					//   getProfileWithNoAds();
				} else {
					// props.getUser(null);
				}
	...
	async function getChatId(id) {
		...
		await fetch(`https://chat.spapp.click/api/userChatInfo?user_id=${newid}`, requestOptions)
			.then((response) => response.text())
			.then((result) => {
