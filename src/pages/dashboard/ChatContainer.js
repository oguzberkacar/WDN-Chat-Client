...
import { getUser, isLogining, setFullUser, setOnlineUsers, updateLastMessages, addLoggedinUser, activeUser, setchatList, addNewChat, updateChatListMessage, setActiveChatId, deleteChat, changeLastMessage, updateSignleMessage, setConnectionData } from '../../redux/actions';
...
function ChatContainer(props) {
	...
	async function getChats(id) {
		const response = await fetch(`https://chat.spapp.click/api/userChatList?user_id=${id}`, {
			method: 'GET',
			headers: {
	...
	async function getChatDetail(chatid, id, index, last_message) {
		const response = await fetch(`https://chat.spapp.click/api/chatDetail?chat_id=${chatid}&user_id=${id}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application',
			},
