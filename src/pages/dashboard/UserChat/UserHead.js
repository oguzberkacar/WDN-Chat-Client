...
import { openUserSidebar, setFullUser, activeUser, updateLastMessages, deleteChat, setActiveChatId } from '../../../redux/actions';
...
function UserHead(props) {
	...
	async function deletechat(e) {
		...
		const response = await fetch(`https://chat.spapp.click/api//userChatDelete?chat_id=${props.activeChatId}&user_id=${props.user.id}`, {
			method: 'GET',
			headers: {
						...
						<div className="d-flex align-items-center">
									...
									{avatar !== null ? (
										...
										<div className="chat-user-img align-self-center me-3">
											<div className="avatar-xs">
												<span className="avatar-title rounded-circle wdncolor text-white">{name.charAt(0)}</span>
											</div>
										</div>
									)}
								</div>
							...
							<div className="flex-grow-1 overflow-hidden">
								<h5 className="font-size-16 mb-0 text-truncate">
									<Link id="profile-receiver" to="#" className="text-reset user-profile-show pb-2">
										<p className="p-0 m-0 d-flex justify-items-center align-items-cente gap-2">
											{name}
											{!isGroup && <>{country !== 'NULL' && country !== undefined && country !== null ? <i className="ri-user-star-line"></i> : <i className="ri-user-line"></i>}</>}
										</p>
									</Link>
									{!isUserLoggedIn && (
										<UncontrolledTooltip target="profile-receiver" placement="bottom-start">
								...
								{isUserLoggedIn && !isGroup && (
									<span className="text-muted mb-0 ">
										{age !== null || (age !== 'NULL' && <p className="m-0">Age: {age}</p>)}
											...
											<div>
												{lastSeen !== undefined && lastSeen !== null && (
													<div className="d-flex justify-items-center align-items-center">
														<p className="m-0 p-0">
															Last Seen: <Moment date={lastSeen} format="DD/MM hh:mm" />
														</p>
													</div>
												)}
											</div>
