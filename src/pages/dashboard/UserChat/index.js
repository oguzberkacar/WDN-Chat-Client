...
//actions
import { openUserSidebar, setFullUser, updateLastMessages, setchatList, addNewChat, updateChatListMessage, setActiveChatId, changeLastMessage, deleteChat, setOldMessageLoading, oldMessageSuccess, setOldMessageError } from '../../../redux/actions';
...
import { SendMessageToSocket, GetMoreMessage, SendChatRoomMessageToSocket } from '../../../helpers/socket';
...
function UserChat(props) {
	...
	useEffect(() => {
		...
		if (chat !== undefined && chat.messages !== undefined && chat.chatroom == undefined) {
			setIsChannel(false);
	...
	const GetMoreMessage = async (page) => {
		...
		if (!isChannel) {
			await fetch(`https://chat.spapp.click/api/chatDetail?chat_id=${chatid}&user_id=${user.id}&page=${page}`, {
				method: 'GET',
				headers: {
			...
			await fetch(`https://chat.spapp.click/api/chatRoomMessages?chat_room_chat_id=${chatid}&page=${page}`, requestOptions)
				.then((response) => response.json())
				.then((responseJson) => {
					setIsloading(true);
					let obj = responseJson;
				...
				<React.Fragment>
					<div className="user-chat w-100 ">
						<div className="d-flex ">
							<div className={`${props.userSidebar ? 'w-70' : 'w-100'} d-flex flex-column vh-100`}>
								{/* render user head */}
								<UserHead deletechat={props.deleteChat} chat={messages} />

								<SimpleBar style={{ maxHeight: '100%' }} className="chat-conversation p-3 p-lg-4 flex-grow-1" id="messages">
									...
									{props.oldMessageError !== null && <p className="text-center">{props.oldMessageError}</p>}
									<ul className="list-unstyled mb-0">
										{messages !== undefined &&
											messages.map((chat, key) => (
												<div key={key}>
													{key == 0 && messages.length > 9 ? (
														<li key={key} ref={lastPostRef} className={(props.user !== null && chat.away_user && chat.away_user.user_id == props.user.id) || (chat.from_id && chat.from_id == props.user.id) ? 'right' : ''}>
															<div className="conversation-list">
																<div className="user-chat-content">
																	<div className="ctext-wrap">
																		<div className={`ctext-wrap-content ${props.user !== null && chat.from_id !== props.user.id && 'wdncolor wdnborder'}`}>
																			{' '}
																			{chat.message && <p className="mb-0">{chat.message}</p>}
																			<p className="chat-time mb-0">
																				<i className={`${chat.is_sendToOthers == 0 ? 'ri-check-fill' : 'ri-check-double-fill'} align-middle ${chat.is_seen == 1 && 'text-primary'}`}></i>
																				<i className="ri-time-line align-middle"></i>
																				<span className="align-middle">
																					<Moment date={chat.created_at} format="DD-MM hh:mm" />
																				</span>
																			</p>
																		</div>
																		...
																		<UncontrolledDropdown className="align-self-start">
																			...
																			<DropdownMenu>
																				<DropdownItem onClick={() => navigator.clipboard.writeText(chat.message)}>
																	...
																	<div className="conversation-name">{props.user !== null && chat.from_id === props.user.id ? 'You' : chat.name}</div>
																</div>
															</div>
														</li>
													) : (
														<li key={key} className={(props.user !== null && chat.away_user && chat.away_user.user_id == props.user.id) || (chat.from_id && chat.from_id == props.user.id) ? 'right' : ''}>
															<div className="conversation-list">
																<div className="user-chat-content">
																	<div className="ctext-wrap">
																		<div className={`ctext-wrap-content ${props.user !== null && chat.from_id !== props.user.id && 'wdncolor wdnborder'}`}>
																			...
																			<p className="chat-time mb-0">
																				<i className={`${chat.is_sendToOthers == 0 ? 'ri-check-fill' : 'ri-check-double-fill'} align-middle ${chat.is_seen == 1 && 'text-primary'}`}></i>
																				<i className="ri-time-line align-middle"></i>
																				<span className="align-middle">{chat.created_at !== null && <Moment date={chat.created_at} format="DD-MM hh:mm" />}</span>
																			</p>
																		</div>
																	...
																	<div className="conversation-name">{props.user !== null && chat.from_id === props.user.id ? 'You' : chat.away_user.showname + ' / ' + chat.away_user.age}</div>
																</div>
															</div>
														</li>
													)}
					...
					<div className="user-chat w-100 ">
						<div className="d-flex ">
							<div className={`${props.userSidebar ? 'w-70' : 'w-100'} d-flex flex-column vh-100`}>
								...
								<SimpleBar style={{ maxHeight: '100%' }} ref={ref} className="chat-conversation p-3 p-lg-4 flex-grow-1 position-relative" id="messages">
									{' '}
									{props.oldMessageLoading && (
										<div className="d-flex justify-content-center align-items-center mb-3">
											<div className="spinner-border" role="status">
									...
									<ul className="list-unstyled mb-0">
										{messages !== undefined &&
											messages.map((chat, key) => (
												<div key={key}>
													{key == 0 && messages.length > 9 ? (
														<li key={key} ref={lastPostRef} className={props.user !== null && chat.from_id == props.user.id ? 'right' : ''}>
															<div className="conversation-list">
																<div className="user-chat-content">
																	<div className="ctext-wrap">
																		<div className={`ctext-wrap-content ${props.user !== null && chat.from_id !== props.user.id && 'wdncolor wdnborder'}`}>
														...
														<li key={key} className={props.user !== null && chat.from_id == props.user.id ? 'right' : ''}>
															<div className="conversation-list">
																<div className="user-chat-content">
																	<div className="ctext-wrap">
																		<div className={`ctext-wrap-content ${props.user !== null && chat.from_id !== props.user.id && 'wdncolor wdnborder'}`}>
																	...
																	<div className="conversation-name">{props.user !== null && chat.from_id === props.user.id ? 'You' : chat.showname}</div>
																</div>
															</div>
														</li>
													)}
